...
if __name__ == "__main__":
    ...
    for iter_idx in range(args.self_training_iters):
        print('-' * 40)
        print('Self-training Iteration {}: '.format(iter_idx + 1))
        ...
        dev_dataset = NegationDataset(examples=dev_examples,
                                      labels=[0 if i == -1 else i for i in dev_label_ids],
                                      max_length=args.max_length,
                                      pretrained_tokenizer=tokenizer)
        ...
        pred_dev_label_ids = np.argmax(pred_dev_logits, axis=1)  # get the pseudo labels
        ...
        selected_pred_labels_ids = [pred_dev_label_ids[i] for i in indices]  # use pseudo labels

        # Iteratively build up the dataset
        if eval(args.iteratively_build_train_data):
            # Add then to train
            train_examples += selected_examples
            train_label_ids += selected_pred_labels_ids
