...
# Tunable
parser.add_argument('--iteratively_train_model', type=str, default='True')
parser.add_argument('--iteratively_build_train_data', type=str, default='False')
...
parser.add_argument('--self_training_iters', type=int, default=30)
...
if __name__ == "__main__":
    ...
    for iter_idx in range(args.self_training_iters):
        print('\n\n')
        print('-' * 40)
        print(f"Self-training iteration {iter_idx + 1}")

        # Stop training if there is no dev examples left
        if len(dev_input_ids) == 0:
        ...
        pred_label_ids = np.argmax(pred_dev_logits, axis=2)

        # Get the max probability for each predicted token probability vector
        pred_dev_probas_max = np.amax(pred_dev_probas, axis=2)
        ...
        # Get the average probability for each sentence
        pred_dev_probas_avg = []
        for sent_length, probas_list in zip(sents_length, pred_dev_probas_max.tolist()):
            pred_dev_probas_avg.append(sum(probas_list[1:sent_length + 1]) / sent_length)

        # Save the current indices as previous indices
        prev_indices = deepcopy(indices)

        # Get the sentence with the average probability higher than threshold
        indices = np.where(np.array(pred_dev_probas_avg) > args.st_threshold)[0]
        ...
        selected_input_ids = [dev_input_ids[i] for i in indices]
        selected_attention_mask = [dev_attention_mask[i] for i in indices]
        selected_label_ids = [torch.from_numpy(pred_label_ids[i]) for i in indices]
        ...
        if eval(args.iteratively_build_train_data):
            ...
            dev_input_ids = [i for j, i in enumerate(dev_input_ids) if j not in indices]
            dev_attention_mask = [i for j, i in enumerate(dev_attention_mask) if j not in indices]
        ...
        _, _, _, preds_to_save = evaluate(trained_trainer=trainer,
                                          ...
                                          data_provider=test_set_provider)
        # Save the flattened preds
        with open(os.path.join(current_iteration_output_path, 'flattened_source_preds.pkl'), 'wb') as f:
            pickle.dump(preds_to_save, f)
